...
void testForCheck(struct Tile *GameBoard){ //needs finished
    for (int i = 0; i<64; i++) {
...
void Move(struct Tile *GameBoard, int start, int end){
    ...
    for (int i = 0; i<32; i++){
        if (threat[i] == end){ //move successful
            verifyMove = 1;
            if (GameBoard[start].pieceType == 'P') {
                if (end > 55 || end < 8){
                    promotePawn(&GameBoard[start]);
                }
                if (enPassant == end){ //en passant move
                    verifyMove = 2;
                } //en passant move
                if (GameBoard[start].moved == 0 && GameBoard[start-(16*GameBoard[start].pieceColor)].pieceType == 'U'){
                    enPassantBuffer = 1;
                    enPassant = start-(8*GameBoard[start].pieceColor);
                } //enable en passant
            }
            if (GameBoard[start].pieceType == 'K') { //king moves
                verifyMove = 3;
            }
            //add castling
        } 
